...
// Kolor nagłówka
const HEADER_COL = '#ea7835';

// Tytuł do headerów
const TITLE = 'zapis niezależnych zakresów';
...
const createWhereToPrintFiles = () => {
	...
	getFolders(getIdFromUrl(EXPERIMENT_DIR)).forEach(folder => {
		...
		getSheet('Wyniki', fileId)
			...
			.getSheet()
			.getRange('C3')
			.setValue(shorter[folderType].toUpperCase())
			.getSheet()
			.getParent()
			.getSheets()
...
/**
 * Krok 4
 * Aktualizuje linki do składowych arkuszy w arkuszu z podsumowanie
 */

const updateLinksToImportrange = () => {
	const sheet = getSheet('Wyniki');
	sheet
		.getRange('B17')
		.setFormula(
			`=IMPORTRANGE("${WHERE_TO_PRINT.geo.ext}";"Wyniki!C3:I12")`
		);

	sheet
		.getRange('K17')
		.setFormula(
			`=IMPORTRANGE("${WHERE_TO_PRINT.geo.loc}";"Wyniki!C3:I12")`
		);
	sheet

		.getRange('T17')
		.setFormula(
			`=IMPORTRANGE("${WHERE_TO_PRINT.geo.hub}";"Wyniki!C3:I12")`
		);
};
...
const setTitle = () => {
	// File
	const fileUrl =
		'https://docs.google.com/spreadsheets/d/1RGJnjQ86VKcf0kx1F4aUjunosaZ_Y0-e9pSTBU7AeTk/edit#gid=717913167';
